'use client'

import { Label, TextInput } from 'flowbite-react'
import React, { SVGProps } from 'react'
import Button from '../Button/Button';

const MailIcon: React.FC<SVGProps<SVGSVGElement>> = (props) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6" {...props}>
        <path stroke-linecap="round" stroke-linejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
    </svg>
);

const NewsletterForm: React.FC = () => {
    return (
        <div className='text-right space-y-4'>
            <h2 className='text-2xl font-semibold'>Newsletter Subsribe</h2>
            <p>Get instant updates about our new products and special promos!</p>
            <form className='relative'>
                <TextInput
                    icon={MailIcon}
                    id="email"
                    placeholder="Your email address"
                    required
                    type='email'
                />
                <button className='absolute right-0 top-0 h-full bg-black text-white px-4 rounded-r-lg' type='submit'>Submit</button>
            </form>
        </div>
    )
}

export default NewsletterForm